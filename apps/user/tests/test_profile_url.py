...
class ProfileImageTestCase(APITestCase):
    ...
    def generate_image_file(self):
        ...
        file.seek(0)
        return SimpleUploadedFile("test_profile.jpg", file.read(), content_type="image/jpeg")
    ...
    def test_upload_profile_image_success(self):
        ...
        self.assertTrue(self.user.profile_img_url.startswith('/media/profile_images/'))  # 경로 확인
    ...
    def test_overwrite_profile_image(self):
        ...
        response = self.client.post(self.url, {"profile_image": image2}, format='multipart')

        # 검증
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.user.refresh_from_db()
        self.assertNotEqual(first_url, self.user.profile_img_url)  # URL이 바뀌었는지 확인
    ...
    def test_upload_without_authentication(self):
        ...
        image = self.generate_image_file()
        response = self.client.post(self.url, {"profile_image": image}, format='multipart')
